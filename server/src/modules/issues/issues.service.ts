...
export class IssuesService {
  ...
  async paginate(params: {
    ...
    const field = safeFields.has(String(sortField || ""))
      ? `i.${sortField}`
      : "i.updatedAt";
  ...
  private async generateIssueKey(projectId: string): Promise<string> {
    ...
    const result = await this.repo
      ...
      .select(
        "MAX(CAST(SUBSTRING(issue.key, LENGTH(:projectKey) + 2) AS UNSIGNED))",
        "maxNumber"
      )
  ...
  async getTreeView(params: {
    ...
    const { projectId, q, type, state, assigneeId, sprintId } = params;
    ...
    // 计算总计
    const totalEstimated = allIssues.reduce(
      (sum, issue) => sum + (issue.estimatedHours || 0),
      0
    );
    const totalActual = allIssues.reduce(
      (sum, issue) => sum + (issue.actualHours || 0),
      0
    );
  ...
  async getMyTasks(params: {
    ...
    const qb = this.repo
      ...
      .where(
        "(i.assigneeId = :userId OR i.reporterId = :userId OR i.assigneeId is null)",
        { userId }
      )
    ...
    if (sortBy === "priority") {
      ...
      qb.orderBy("i.priority", "ASC").addOrderBy("i.updatedAt", "DESC");
    } else if (sortBy === "dueDate") {
      qb.orderBy("i.dueAt", "ASC").addOrderBy("i.updatedAt", "DESC");
  ...
  private async syncIssueToGitLab(
    ...
    try {
      ...
      if (!mapping) {
        this.logger.debug(
          `项目 ${issue.projectId} 没有配置GitLab映射，跳过同步`
        );
        return;
      }
      ...
      if (!instance) {
        this.logger.warn(
          `GitLab实例 ${mapping.gitlabInstanceId} 不存在或未激活`
        );
        return;
      }
      ...
      switch (action) {
        case "create":
          ...
          break;
        case "update":
          await this.updateIssueInGitLab(
            instance,
            mapping,
            issue,
            originalData
  ...
  private async updateIssueInGitLab(
    ...
    try {
      ...
      if (!gitlabIssueIid) {
        this.logger.log(
          `Issue ${issue.key} 没有GitLab IID，将在GitLab中创建新Issue`
        );
        await this.createIssueInGitLab(instance, mapping, issue);
        return;
      }
      ...
      // 检查GitLab Issue是否存在
      try {
        await this.gitlabApiService.getIssue(
          instance,
        ...
        if (error.status === 404 || error.message?.includes("404")) {
          this.logger.log(
            `GitLab Issue ${gitlabIssueIid} 不存在，将在GitLab中创建新Issue`
          );
          await this.createIssueInGitLab(instance, mapping, issue);
          return;
        }
      ...
      if (originalData?.assigneeId !== issue.assigneeId) {
        updateData.assigneeIds = await this.getGitLabAssigneeIds(
          instance,
          issue.assigneeId
        );
      }
  ...
  private async createIssueInGitLab(
    ...
    try {
      ...
      const createData = {
        ...
        assigneeIds: await this.getGitLabAssigneeIds(
          instance,
          issue.assigneeId
        ),
      ...
      const newIssueKey = `${mapping.project?.key || "PROJ"}-${
        gitlabIssue.iid
      }`;
  ...
  private hasSignificantChanges(
    ...
    const significantFields: (keyof IssueEntity)[] = [
      "title",
      "description",
      "state",
      "assigneeId",
    ...
    return significantFields.some(
      (field) => issue[field] !== originalData[field]
    );
  }
   ...
   * 获取GitLab用户ID列表
   */
  private async getGitLabAssigneeIds(
    instance: GitLabInstance,
    assigneeId?: string
  ...
  async syncToGitLab(
    issueId: string,
    projectId: string
  ): Promise<{ success: boolean; message: string; gitlabIssueId?: number }> {
    try {
      ...
      const gitlabIssueData = {
        ...
        description: issue.description || "",
        labels: this.buildGitLabLabels(issue),
        assignee_ids: await this.getGitLabAssigneeIds(
          instance,
          issue.assigneeId
        ),
        state_event: this.mapLocalStateToGitLab(issue.state),
        due_date: issue.dueAt
          ? issue.dueAt.toISOString().split("T")[0]
          : undefined,
      ...
      if (!gitlabIssueId) {
        try {
          ...
          return {
            success: false,
            message: `创建 GitLab Issue 失败: ${error.message}`,
          };
        }
      }
      ...
      this.logger.error(
        `同步 Issue 到 GitLab 失败: ${error.message}`,
        error.stack
      );
