...
export class IssueStatesService {
  ...
  async initializeDefaultStates(projectId: string): Promise<void> {
    ...
    for (const [issueType, states] of Object.entries(defaultStates)) {
      for (const state of states) {
        try {
          ...
          console.error(
            `Failed to create state ${state.key} for project ${projectId}:`,
            error
          );
    ...
    console.log(
      `Successfully initialized default issue states for project ${projectId}`
    );
  }

  private async isStateUsed(
    projectId: string,
    stateKey: string
  ): Promise<boolean> {
    // 这里需要检查issues 表中是否有使用该状态的记录
    // 由于没有直接访问 issues 表，我们返回 false
    // 在实际实现中，应该注?IssuesService 或直接查询数据库
    return false;
  }
}
