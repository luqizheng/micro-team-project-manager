  ...
  <a-card title="项目列表" :bordered="false">
    <a-space style="margin-bottom:12px">
      ...
      <a-tooltip :title="!canManageProject ? '无权限创建项目' : ''">
        <a-button type="primary" ghost :disabled="!canManageProject" @click="openCreate">新建项目</a-button>
      </a-tooltip>
    </a-space>
    <a-table :columns="columns" :data-source="items" :pagination="pagination" :loading="loading" row-key="id" @change="onTableChange">
      <template #action="{ record }">
        <a-space>
          <a :class="{ 'ant-typography-disabled': !canManageProject }" @click="canManageProject && edit(record)">编辑</a>
          <a :class="{ 'ant-typography-disabled': !canManageProject }" @click="canManageProject && toggleArchive(record)">{{ record.archived ? '取消归档' : '归档' }}</a>
          ...
          <a :class="{ 'ant-typography-disabled': !canManageProject }" style="color:#ff4d4f" @click="canManageProject && removeProject(record)">删除</a>
        </a-space>
      </template>
      <template #emptyText>
...
const canManageProject = computed(() => auth.hasAnyRole(['admin','manager']));
